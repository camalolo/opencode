...
describe("public event manifest", () => {
  ...
  test("uses canonical definitions for current public events", () => {
    ...
    expect(EventManifest.Latest.get("project.list.updated")).toBe(Project.Event.ListUpdated)
    expect(Project.Event.Definitions).toEqual([Project.Event.Updated, Project.Event.ListUpdated])
    ...
    // Ordering snapshot: the revert events sit between the SessionV1 block and
    // the feature definitions; project.list.updated follows project.updated.
    expect(EventManifest.Definitions.slice(40, 43).map((definition) => definition.type)).toEqual([
      "session.next.revert.staged",
      "session.next.revert.cleared",
      "session.next.revert.committed",
    ])
    expect(EventManifest.Definitions[74]).toBe(Project.Event.ListUpdated)
